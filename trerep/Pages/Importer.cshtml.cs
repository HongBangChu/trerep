...
namespace trerep.Pages
{
    public class ImporterModel : PageModel
    {
        ...
        public ImporterModel(IConfiguration configuration, IHostingEnvironment hostingEnvironment)
        {
        ...
        public JsonResult OnPostFxTran()
        {
            String result = "";
                ...
                using (var stream = new FileStream(fullPath, FileMode.Create))
                {
                    ...
                    using (var conn = new NpgsqlConnection(_connStr))
                    {
                        ...
                        for (int i = (sheet.FirstRowNum + 1); i <= sheet.LastRowNum; i++) //Read Excel File
                        {
                            ...
                            para.AddProperty("month", int.Parse(Request.Query["month"]));
                            para.AddProperty("year", int.Parse(Request.Query["year"]));
                            para.AddProperty("abc", 1);
                            ...
                            using (var cmd = new NpgsqlCommand("data.fxtran_import_row", conn))
                            {
                                ...
                                cmd.ExecuteNonQuery();
                                result += (result.Length==0) ? outParam.Value : "," + outParam.Value;
                            }
                        }
                    }
                }
